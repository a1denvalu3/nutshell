        ...
        for d in range(0, 256):
            ...
            for p_idx in parent_indices:
                ...
                parent_hash = hashlib.sha256(
                    ...
                    left_node[1].to_bytes(8, 'big') + right_node[1].to_bytes(8, 'big')
                ).digest()
...
async def build_trees_for_keyset_at_timestamp(
    ...
    """
    logger.debug(f"Building Sparse Merkle Sum Trees for keyset {keyset_id} (timestamp limit: {timestamp_limit})")
    ...
    logger.debug(f"Loaded {len(promises_rows)} promises and {len(proofs_rows)} spent proofs from the database.")
    ...
    logger.debug(f"Constructed Issued Tree with {len(issued_leaves)} leaves and Spent Tree with {len(spent_leaves)} leaves.")
    
    return issued_tree, spent_tree


async def update_pol_manifests(ledger) -> None:
    """
    ...
    logger.info(f"Publishing synchronized PoL epoch {next_epoch_index} for all keysets...")
    ...
    for keyset_id, keyset in ledger.keysets.items():
        ...
        try:
            issued_tree, spent_tree = await build_trees_for_keyset_at_timestamp(ledger, keyset_id, current_time)
            root_issued_hash, root_issued_sum = issued_tree.root
            root_spent_hash, root_spent_sum = spent_tree.root
            keyset_results[keyset_id] = (root_issued_hash, root_issued_sum, root_spent_hash, root_spent_sum, keyset)
        except Exception as e:
            logger.error(f"Failed to build trees for keyset {keyset_id}: {e}")
            
    if not keyset_results:
        logger.debug("No active or unexpired keysets found to commit. Skipping epoch publication.")
        return
    ...
    try:
        if settings.mint_pol_mock_ots:
            ...
            ots_receipt = b"\x00" * 8 + b"MOCK_OTS_RECEIPT_FOR_HASH_" + global_digest.hex().encode('utf-8')
        else:
