...
async def generate_output_receipt(
    ledger, keyset_id: str, amount: int, b_hex: str
) -> PolReceipt:
    ...
    try:
        ...
        priv_bytes = hashlib.sha256(
            f"fallback_pol_seed_{keyset_id}_{amount}".encode("utf-8")
        ).digest()
...
async def generate_spent_receipt(
    ledger, keyset_id: str, amount: int, y_hex: str
) -> PolReceipt:
    target_epoch = await get_target_epoch(ledger)
