...
@click.option("--host", "-h", default="localhost", help="Mint address.")
@click.option("--port", "-p", default=8086, help="Mint gRPC port.")
@click.option(
    "--insecure", "-i", is_flag=True, default=False, help="Connect without mutual TLS."
)
@click.option(
    "--ca-cert-path",
    "-ca",
    ...
    if not insecure:
        ...
        for what, path in [
            ...
            if not path or not os.path.exists(path):
                click.echo(
                    f"Error: Couldn't get {what}. The path '{path}' does not exist.",
                    err=True,
                )
                ctx.exit(1)

        with open(client_key_path, "rb") as key_file, open(
            client_cert_path, "rb"
        ) as cert_file, open(ca_cert_path, "rb") as ca_file:
...
def update_short_description(ctx: Context, description: str):
    stub = ctx.obj["STUB"]
    try:
        stub.UpdateShortDescription(
            management_pb2.UpdateDescriptionRequest(description=description)
        )
...
def update_lightning_fee(
    ctx: Context, fee_percent: Optional[float], min_fee_reserve: Optional[int]
):
    stub = ctx.obj["STUB"]
    try:
        stub.UpdateLightningFee(
...
def get_quote_ttl(ctx: Context, quote_id: str):
    stub = ctx.obj["STUB"]
    try:
        response = stub.GetQuoteTtl(
            management_pb2.GetQuoteTtlRequest(quote_id=quote_id)
        )
...
def rotate_next_keyset(
    ...
    try:
        ...
        click.echo(
            f"New keyset successfully created:\n{keyset.id = }\n{keyset.unit = }\n{keyset.max_order = }\n{keyset.input_fee_ppk = }\n{keyset.final_expiry}"
        )
    except grpc.RpcError as e:
        click.echo(f"Error: {e.details()}", err=True)
