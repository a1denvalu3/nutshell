...
pytestmark = pytest.mark.skipif(
    not is_deprecated_api_only, reason="Deprecated API not enabled"
)
...
def test_fuzz_deprecated_melt(client, pr, proofs, outputs):
    ...
    payload = {"pr": pr, "proofs": inputs_json, "outputs": outputs_json}
...
@given(proofs=st.lists(proof_strategy, min_size=1, max_size=10))
...
def test_fuzz_deprecated_split(client, proofs, outputs, amount):
    ...
    payload = {"proofs": inputs_json, "outputs": outputs_json}
