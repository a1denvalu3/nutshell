...
def test_fuzz_mint(client, quote, outputs, signature):
    ...
    payload = {"quote": quote, "outputs": outputs_json, "signature": signature}
...
def test_fuzz_melt_quote(client, unit, request, options):
    payload = {"unit": unit, "request": request, "options": options}
...
@given(
    ...
    outputs=st.one_of(
        st.none(), st.lists(blinded_message_strategy, min_size=1, max_size=5)
    ),
)
def test_fuzz_melt(client, quote, inputs, outputs):
    ...
    payload = {"quote": quote, "inputs": inputs_json, "outputs": outputs_json}
...
def test_fuzz_swap(client, inputs, outputs):
    ...
    outputs_json = [o.model_dump() for o in outputs]
    payload = {"inputs": inputs_json, "outputs": outputs_json}
...
@given(Ys=st.lists(st.one_of(hex_string(), public_key()), min_size=1, max_size=20))
...
@given(outputs=st.lists(blinded_message_strategy, min_size=1, max_size=20))
