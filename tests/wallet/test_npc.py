...
async def test_create_lnurl_success(npc):
    with patch("cashu.wallet.npc.httpx.AsyncClient") as mock_client:
        ...
        mock_patch_resp.json.return_value = {
            "error": False,
            "data": {"user": {"mintUrl": "https://mint.example.com"}},
        }
...
async def test_create_lnurl_already_exists(npc):
    with patch("cashu.wallet.npc.httpx.AsyncClient") as mock_client:
        ...
        mock_info_resp.json.return_value = {
            "error": False,
            "data": {"user": {"mintUrl": "https://existing.mint"}},
        }
...
async def test_mint_quotes(npc, mock_wallet):
    with patch("cashu.wallet.npc.httpx.AsyncClient") as mock_client, patch(
        "cashu.wallet.npc.get_bolt11_mint_quote", new_callable=AsyncMock
    ) as mock_get_quote:
        mock_instance = mock_client.return_value.__aenter__.return_value
        ...
        mock_wallet.mint = AsyncMock(
            return_value=[Proof(id="1", amount=100, C="C", secret="s")]
        )
