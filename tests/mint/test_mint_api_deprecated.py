...
async def test_api_keyset_keys(ledger: Ledger):
    response = httpx.get(f"{BASE_URL}/keys/016d1ce32977b2d8a340479336a77dc18db8da3e782c5083a6f33d70bc158056d1")
    assert response.status_code == 200, f"{response.url} {response.status_code}"
    assert ledger.keyset.public_keys
    assert response.json() == {
        str(k): v.format().hex() for k, v in ledger.keyset.public_keys.items()
...
async def test_mint(ledger: Ledger, wallet: Wallet):
    ...
    assert result["promises"][0]["id"] == "016d1ce32977b2d8a340479336a77dc18db8da3e782c5083a6f33d70bc158056d1"
